...
<div id="dialogo_generar_factura" title="Generar Factura">
    ...
    <li style="float:left">
        ...
        <input type="text" name="numero_pedido" id="numero_pedido" readonly="readonly" class="required" size="10"/>
    </li>
    <li style="float:left">
...
<div id="listado">
    ...
    <tbody class="scroll">
<?php
	$resultado = $bbdd->ejecuta_consulta("SELECT p.id, p.numero_pedido, p.id_cliente, p.fecha, p.estado, p.importe " .
		"FROM pedidos p, clientes c " .
		"WHERE p.id_cliente = c.id AND (p.numero_pedido LIKE '%" . $busqueda_rapida . "%' OR CONCAT(c.nombre,c.apellidos) LIKE '%" . $busqueda_rapida . "%')" .
		"ORDER BY c.nombre_completo, p.fecha");
